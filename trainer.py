    ...
    def train(self):
        ...
        for epoch in range(self.opt.max_epoch):
            ...
            for ii, (imgs, _) in enumerate(progressbar):
                ...
                # fake_img = output[:, :3, :, :]
                # seg_ = output[:, 3:, :, :]
                # seg_image = seg_.max(1)[1].unsqueeze(1)
                # fake_output = self.netd(fake_img)
                # target = target.long()
                ...
                if self.opt.debug:
                    ...
                    # segs = torch.cat((seg_image, target.type_as(seg_image)), 0)
                    # segs = segs.type(torch.float32)
                    # tv.utils.save_image(segs, os.path.join(self.opt.save_path, 'train', '{}_seg.jpg'.format(ii)),
                    #                     normalize=True,
                    #                     range=(-1, 1))
    ...
    def validate(self):
        ...
        for ii, (imgs, _) in enumerate(progressbar):
            ...
            if self.opt.debug:
                ...
                # segs = torch.cat((seg_image, target.type_as(seg_image)), 0)
                # segs = segs.type(torch.float32)
                # tv.utils.save_image(segs, os.path.join(self.opt.save_path, 'val', '{}_seg.jpg'.format(ii)),
                #                     normalize=True,
                #                     range=(-1, 1))
